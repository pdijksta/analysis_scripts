...
sp2.fill_between(t_bun[:-1]/25e-9, t_bun_zeros, drift_contribution_per_hc, alpha=0.4, color='red', label='Drift')
sp2.fill_between(t_bun[:-1]/25e-9, drift_contribution_per_hc, drift_contribution_per_hc+quad_contribution_per_hc, alpha=0.4, color='blue', label='Quad')
sp2.plot(meas_loss_per_hc, '.-', color='k', label='Measured', marker=None)
...
for coast_ctr, coast_str in enumerate(coast_strs):
    for sey_ctr, sey in enumerate(sey_vect):
        ...
        try:
            ...
            ms.sciy()
            sp2.plot(t_bun[:-1]/25e-9, sim_loss_per_hc, '.-', color=color_curr, label=label, ls=coast_linestyle_dict[coast_str], marker=None)

        except IOError as err:
            print('Got:', err)

sp1.legend(bbox_to_anchor=(1, 1.02),  loc='upper left')
sp2.legend(bbox_to_anchor=(1, 1.02),  loc='upper left', title='Dip SEY')

# Figures for devices only
